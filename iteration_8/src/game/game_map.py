...
class Map(object):
    ...
    """
    def __init__(self, name, number_of_case, enemies=None, boxes=None):
        if enemies is None:
            enemies = []
        if boxes is None:
            boxes = []
    ...
    def get_case_content(self, case):
        """
        :param case: index de self_map_list(int)
        :return: str de ce que contient la case(name, image, attack_level)
        """
